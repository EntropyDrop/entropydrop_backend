...
from pipeline_registry import SKING_DDJ_V61B, SKING_DDJ_V104, SKING_DDJ_V101C, SKING_DDJ_V104B
...
def test_get_models(client):
    ...
    assert SKING_DDJ_V101C in data["image_to_skin_models"]
    assert SKING_DDJ_V104B not in data["image_to_skin_models"]
...
def test_get_generation_credit_cost_with_params(client):
    ...
    backend_utils.redis_conn.set("config:model_price:SKING_DDJ_v101c", "3")
    backend_utils.redis_conn.set("config:model_price:z_image", "2")

    try:
        # Test model_version only
        response = client.get("/skin/api/generation_credit_cost?model_version=SKING_DDJ_v101c")
        assert response.status_code == 200
        assert response.json() == {"credits": 3, "is_pro": False, "under_maintenance": False}

        # Test aux_model_version + model_version
        response = client.get("/skin/api/generation_credit_cost?aux_model_version=z_image&model_version=SKING_DDJ_v101c")
        assert response.status_code == 200
        assert response.json() == {"credits": 5, "is_pro": False, "under_maintenance": False}
    finally:
...
def test_get_generation_credit_cost_pro_exclusive(client):
    import backend_utils
    backend_utils.redis_conn.set("config:model_price:SKING_DDJ_v101c", "3")
    backend_utils.redis_conn.set("config:model_pro:SKING_DDJ_v101c", "1")
...
def test_model_pipeline_mapping_is_immutable():
    assert set(routers.generate.MODEL_PIPELINES) == {SKING_DDJ_V101C, SKING_DDJ_V104B}
    ...
    assert pipeline.dense_uv_checkpoint_file == "SKING_DDJ_v101c/parser.pt"
...
@pytest.mark.parametrize("model_version", [SKING_DDJ_V101C, SKING_DDJ_V104B])
...
@pytest.mark.parametrize("model_version", [SKING_DDJ_V101C])
...
def test_re_enqueue_if_missing_resumes_accepted_dense_uv_stage_one(
    ...
    assert kwargs["args"][-2] == SKING_DDJ_V101C
...
def test_v101c_spec():
    pipeline = routers.generate.get_pipeline(SKING_DDJ_V101C)
    payload = pipeline.to_task_payload()
    assert payload["prompt_file"] == "real_to_render3.zh-hans.txt"
    assert payload["dense_uv_checkpoint_file"] == "SKING_DDJ_v101c/parser.pt"
    assert payload["provider_model"] == "nano-banana-pro"


def test_v101c_keeps_v104b_generation_settings():
    previous = routers.generate.get_pipeline(SKING_DDJ_V104B).to_task_payload()
    current = routers.generate.get_pipeline(SKING_DDJ_V101C).to_task_payload()
    assert previous.pop("dense_uv_checkpoint_file") == "SKING_DDJ_v104/parser.pt"
    assert current.pop("dense_uv_checkpoint_file") == "SKING_DDJ_v101c/parser.pt"
